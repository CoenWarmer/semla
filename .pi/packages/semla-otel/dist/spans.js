 ...
 *
 * Fork of pi-otel/spans.js with two additions for cross-agent trace correlation:
 *   1. startInteraction(prompt, parentContext?) — accepts an optional OTel context
 *      so subagent interactions can be parented under the workflow tool span.
 *   2. getToolContext(toolCallId) — exposes the OTel context containing the tool
 *      span, so the caller can extract the W3C traceparent to propagate.
 */
import { context as otelContext, SpanStatusCode, trace, } from "@opentelemetry/api";
import { SeverityNumber } from "@opentelemetry/api-logs";
import { ATTR_CONVERSATION_ID, ATTR_ERROR_TYPE, ATTR_GEN_AI_INPUT_MESSAGES, ATTR_GEN_AI_OUTPUT_MESSAGES, ATTR_INPUT_TOKENS, ATTR_OPERATION_NAME, ATTR_OUTPUT_TOKENS, ATTR_PI_CWD, ATTR_PI_SESSION_ID, ATTR_PI_TOOL_CALL_ID, ATTR_PI_TOOL_COUNT, ATTR_PI_TOOL_INPUT, ATTR_PI_TOOL_IS_ERROR, ATTR_PI_TOOL_NAME, ATTR_PI_TOOL_OUTPUT, ATTR_PI_TURN_COUNT, ATTR_PI_TURN_INDEX, ATTR_REQUEST_MODEL, ATTR_RESPONSE_MODEL, ATTR_SESSION_ID, ATTR_SYSTEM, ATTR_TOKEN_TYPE, ATTR_TOOL_CALL_ARGUMENTS, ATTR_TOOL_CALL_ID, ATTR_TOOL_CALL_RESULT, ATTR_TOOL_NAME, clampAttr, EVENT_GEN_AI_ASSISTANT_MESSAGE, EVENT_GEN_AI_CHOICE, EVENT_GEN_AI_TOOL_MESSAGE, EVENT_GEN_AI_USER_MESSAGE, GEN_AI_SYSTEM_PI, SPAN_INTERACTION, SPAN_LLM_REQUEST, SPAN_TURN, spanToolName, } from "./attrs.js";
import { emitLifecycleLog } from "./otel/logs.js";
import { getDurationHistogram, getTokenHistogram, getToolCallsCounter, getToolCallsHistogram, } from "./otel/metrics.js";
/**
...
export class SpanTracker {
    ...
    /**
     * Start a new interaction span. An optional parentContext positions the span
     * as a child of a remote span (e.g. the workflow tool span in the parent agent)
     * enabling cross-agent trace correlation.
     */
    startInteraction(prompt, parentContext) {
        if (this.interaction)
        ...
        const parentCtx = parentContext ?? otelContext.active();
    ...
    endTool(toolCallId, args) {
        ...
        try {
            ...
            if (args.isError)
                counterAttrs[ATTR_ERROR_TYPE] = "tool_error";
    ...
    /**
     * Expose the OTel context for a running tool span so the caller can
     * serialize it as a W3C traceparent and propagate it to subprocesses.
     * Returns undefined if the tool is not currently tracked.
     */
    getToolContext(toolCallId) {
