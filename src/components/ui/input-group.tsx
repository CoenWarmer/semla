...
function InputGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      // Genuine ARIA "group" role for the field plus its addons — not the
      // `<hgroup>` heading-grouping element the name resembles, which is not
      // a valid `role` value at all (aria-role caught this as an error).
...
function InputGroupAddon({
  ...
  return (
    <div
      // No independent semantics: the click below is a convenience for mouse
      // users clicking the padding around an icon/label to focus the nearby
      // input, which a keyboard user already reaches directly via Tab. A
      // "group" role added nothing here beyond the outer InputGroup's own,
      // and made this look like a control needing its own keyboard handling.
      role="none"
...
function InputGroupTextarea({
  ...
  return (
    <Textarea
      data-slot="input-group-control"
      className={cn(
        "flex-1 resize-none rounded-none border-0 bg-transparent p-4 shadow-none ring-0 focus-visible:ring-0 aria-invalid:ring-0 dark:bg-transparent",
        className,
      )}
      {...props}
    />
