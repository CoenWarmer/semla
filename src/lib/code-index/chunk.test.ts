...
async function chunk(content: string, language: IndexLanguage = "typescript") {
  return chunkFile({
    path: "src/a.ts",
    content,
    language,
...
describe("chunkFile", () => {
  it("returns nothing for an empty or whitespace-only file", async () => {
    expect((await chunk(""))).toEqual([]);
    expect((await chunk("\n\n   \n\t\n"))).toEqual([]);
  });

  it("keeps a small file as one chunk covering it", async () => {
    const content = "export function a() {\n  return 1;\n}\n";
    const [only, ...rest] = await chunk(content);

    expect(rest).toEqual([]);
    expect(only).toMatchObject({ path: "src/a.ts", startLine: 1, endLine: 3 });
  });

  it("records the strategy that produced it", async () => {
    // A chunk claiming "ast" was definitely parsed; a language with no grammar
    // falls back to lines and says so, rather than implying a parse.
    expect((await chunk("const a = 1;\n"))[0].strategy).toBe("ast");
    expect((await chunk("# Title\n", "markdown"))[0].strategy).toBe("lines");
  });

  it("carries the file hash onto every chunk", async () => {
  ...
  it("splits a long file into several chunks", async () => {
    const chunks = await chunk(`${block(400, "a")}\n${block(400, "b")}\n${block(400, "c")}`);
    expect(chunks.length).toBeGreaterThan(1);
  });

  it("groups small adjacent blocks rather than emitting a chunk per block", async () => {
    const content = Array.from({ length: 8 }, (_, i) => `const v${i} = ${i};`).join("\n\n");
    expect((await chunk(content))).toHaveLength(1);
  });

  it("keeps a block whole when it fits, rather than cutting to hit the target", async () => {
  ...
  it("cuts a single oversized block on line boundaries", async () => {
    const chunks = await chunk(block(MAX_CHUNK_TOKENS + 400));
  ...
  it("covers the file contiguously and without overlap", async () => {
    const chunks = await chunk(`${block(400, "a")}\n${block(400, "b")}\n${block(400, "c")}`);
  ...
  it("cites line ranges that exist in the file", async () => {
    const content = `${block(400, "a")}\n${block(400, "b")}`;
    const lineCount = content.split("\n").length;

    for (const one of await chunk(content)) {
...
describe("readChunkText round-trip", () => {
  ...
  it("round-trips a file whose blocks are separated by several blank lines", async () => {
  ...
  it("detects an edited file through the hash", async () => {
    const content = "export function one() {\n  return 1;\n}\n";
    const [only] = await chunk(content);
    const edited = content.replace("return 1", "return 2");

    expect(hashContent(readChunkText(edited, only)!)).not.toBe(only.hash);
  });

  it("returns null when the cited range no longer exists", async () => {
    const [only] = await chunk(`${block(50)}`);
    expect(readChunkText("one line\n", { ...only, endLine: 9_999 })).toBeNull();
  });
});

describe("estimateTokens", () => {
  it("scales with length and stays in the right order of magnitude", async () => {
  ...
  it("has a target below its hard ceiling", async () => {
