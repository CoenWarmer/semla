...
export const buildMemoryContextBlock = (
  ...
  const lines = [
    ...
    "",
    "**Workflow subagents have no wiki tools unless you ask for them.**",
    'When a workflow captures sources or writes wiki pages, pass `toolset: "wiki"` in the `workflow` tool call.',
    "Without it subagents get only bash/read/edit/write; they cannot call `wiki_capture_source` and will either fail or hand-roll a capture that silently loses the source's `file_path`.",
    "`wiki_ingest` is deliberately not in that toolset — call it yourself once the capture workflow has finished.",
    "",
