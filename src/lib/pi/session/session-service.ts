...
import {
  applyBranchTarget,
  resolveBranchTarget,
} from "@/lib/pi/session/session-branch";
import { isProjectAnchored, resolveSessionCwd } from "@/lib/pi/session/session-cwd";
import { writeSessionMeta } from "@/lib/pi/session/session-meta";
import { createTurnEventRouter } from "@/lib/pi/session/session-event-router";
import {
  releaseSpanSink,
  retainSpanSink,
...
import type { EmitSessionEvent, PiSessionEvent } from "@/lib/pi/session/session-events";
...
export const runPiPrompt = async ({
  ...
  const extensionReport = buildExtensionLoadReport({
    ...
    // So session-keyed slots are verified for *this* session — a concurrent
    // session's workflow manager must not satisfy this session's check.
    piSessionId: piRuntimeSessionId,
  ...
  const bridgeRunNotifier: BridgeRunNotifier = (runId, opts = {}) => {
    ...
    followBridgeRunProgress(
      readSessionWorkflowManager(piRuntimeSessionId),
      runId,
  ...
  try {
    ...
    // Clear the bridge run notifier so a stale reference can't fire after turn
    // end. Identity-guarded, and only this session's entry: unguarded, the
    // first of two concurrent turns to end wiped the other's notifier, and
