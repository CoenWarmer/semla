 ...
 * (src/lib/pi/session/session-project-attach.ts), and that observation has a
 ...
 *
 * Same shape as src/lib/pi/git/git-status.ts: pure parsers exported for tests,
 * readers that shell out beside them, and the types in a node-free module the
 * client can import.
 */

import { git, gitRaw, gitResult } from "@/lib/pi/git/git";
import {
  CHANGED_FILE_CAP,
  type ChangedFile,
  type ChangeStatus,
