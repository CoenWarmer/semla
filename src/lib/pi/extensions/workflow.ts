 ...
 */
export function sessionFileCwd(
  ...
  try {
    ...
    while (scanned < SESSION_HEADER_SCAN_BYTES) {
      const n = readSync(
        ...
        buffer,
        0,
        Math.min(buffer.length, SESSION_HEADER_SCAN_BYTES - scanned),
        null,
      );
      if (n === 0) {
...
export default function extension(pi: ExtensionAPI) {
  ...
  let pausedForMismatch = runtimeClaim.versionMismatch
  ...
  let manager =
    previousRuntime?.manager ?? new WorkflowManager({ cwd, ...managerOptions });
  ...
  const effort: EffortState =
    (previousRuntime ?? runtimeClaim.versionMismatch)?.effort ??
    createEffortState();
    ...
    if (!armingInstalled) {
      installWorkflowKeywordArming(pi, effort, {
        settingsStore: {
          ...
          save: (nextSettings) =>
            saveWorkflowSettingsForCwd(nextSettings, getCwd()),
        },
      });
