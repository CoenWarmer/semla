...
/**
 * Unforgeable brand for the "this label was generated by one of this file's
 * own built-in quality helpers" flag (see AgentOptions[INTERNAL_LABEL_BRAND]).
 * `Symbol()` (NOT `Symbol.for`, which is a global, string-keyed registry any
 * code — including a workflow script, if it ever got a reference to the
 * registry key — could look up) creates a value that exists ONLY in this
 * module's closure. A workflow script runs inside a separate vm context with
 * no import mechanism and no reference to this module's scope, so it has no
 * syntax that can produce a value `===` to this symbol; the best it can do is
 * set a same-*named* property (if it even knew the property name, which is
 * itself private), which is a different, unequal value. The check below
 * therefore tests identity against this symbol, not mere property presence.
 */
const INTERNAL_LABEL_BRAND: unique symbol = Symbol();

/**
 * Tracks agent() call identity across the attempts of a single retry()/gate()
 * invocation, so agentImpl() can recognize a repeat attempt of the SAME
 * logical call (exempt from the hard-throw) without exempting two distinct
 * agent() calls that merely happen to run while some attempt is in flight.
 *
...
/**
 * Set around a single retry()/gate() attempt's `thunk()` call (a shared
 * {@link RetryAttemptTracker} for all attempts of one retry()/gate()
 * invocation — see that type's doc comment for how it scopes the exemption)
 * so an agent() made from inside it is recognized as a RETRY of the same
 * logical step, not a fresh script-authored label collision. A retried agent
 * must not hard-throw merely for being a retry — attempt 2 of a
 ...
 * contract is "pass a label that varies" instead of "pass a label").
 *
 * agentImpl() disambiguates a legitimate cross-attempt reuse with
 * uniqueRunLabel() (the same numeric-suffix mechanism
 * verify()/judgePanel()/completenessCheck() use) rather than exempting it
 * from uniqueness altogether — every agent() call still gets its own
 * distinct label in onAgentStart/onAgentEnd/the task panel. Deliberately NOT
...
> {
   ...
   * on the script's behalf. Keyed by the module-scoped `INTERNAL_LABEL_BRAND`
   * symbol (not a string), so a workflow script — which runs as plain JS in a
   * separate vm context with no reference to this module's scope — has no
   * way to construct the same key, even if it guessed the string this
   * property would otherwise have had. Setting a same-named *string*
   * property changes nothing, since agentImpl() only ever checks identity
   * against the real symbol. Tells the label-uniqueness check in agent() to
...
export async function runWorkflow<T = unknown>(
  ...
  const agentImpl = async (prompt: string, agentOptions: AgentOptions = {}) => {
    ...
    //
    // `agentOptions[INTERNAL_LABEL_BRAND]` (see uniqueRunLabel() below) is a
    // private flag set ONLY by this file's own built-in helpers (verify,
    // judgePanel, completenessCheck) — unforgeable by a workflow script,
    // since it is keyed by a module-scoped `Symbol()` a sandboxed script has
    // no syntax to reference (see INTERNAL_LABEL_BRAND's doc comment). It
    // exempts a collision from the hard-throw below and instead disambiguates
    // the label with a numeric suffix, because those helpers' labels are
    // runtime-generated on the helper's behalf (the script never chose them
    // and has no way to make them unique itself), whereas a script's own
    // agent() call choosing a label is exactly the case this check exists to
    // hold to a hard, unweakened rule.
    if (!requestedLabel) {
      throw new WorkflowError(
    ...
    const label =
      finalRequestedLabel ||
      defaultAgentLabel(assignedPhase, shared.agentCount);
  ...
  const retry = async (
    ...
    let last: unknown;
    // One tracker for the whole retry() invocation, shared across attempts
    // (see RetryAttemptTracker's doc comment) — `position` resets to 0 at the
    // start of each attempt so the Nth agent() call in the thunk lines up
    // across attempts, while `seenPositions` persists so agentImpl() can tell
    ...
    for (let i = 0; i < attempts; i++) {
      ...
      last = await retryAttemptScope.run(tracker, () => thunk(i));
  ...
  const gate = async (
    ...
    let last: unknown;
    // One tracker for the whole gate() invocation, shared across attempts —
    ...
    for (let i = 0; i < attempts; i++) {
      ...
      last = await retryAttemptScope.run(tracker, () => thunk(feedback, i));
