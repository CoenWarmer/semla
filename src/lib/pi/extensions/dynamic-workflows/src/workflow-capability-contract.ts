...
import { WorkflowCapabilityContractError } from "./errors.ts";
...
export { WorkflowCapabilityContractError } from "./errors.ts";
...
const AGENT_OPTIONS: OptionShape = {
  ...
  options: [
    ...
    option("model", "string", true, null, [
      "always displaced by the phase tier; the override is logged",
    ]),
    option(
      "tier",
      "string",
      ...
      [
        ...
        "required outside any phase; displaced by the phase tier inside one",
      ],
      "model-routes",
    ),
...
const capabilities: readonly CapabilityDescriptor[] = [
  runtimeGlobal("agent", {
    ...
    constraints: [
      ...
      "the declared tier of the phase an agent runs in selects its model; a call-site tier, model, agentType model, or phase/metadata model route is displaced and the override is logged naming the phase, the agent, the ignored value, and the winning tier",
      "an agent outside any declared phase must pass an explicit tier; there is no implicit default tier and a missing one is nonrecoverable",
      "an agent in a phase title that meta.phases never declared is nonrecoverable, because that phase has no tier",
      "a tier that resolves to an unavailable model throws MODEL_NOT_FOUND naming the source (e.g. the tier and what it resolved to) instead of falling back",
  ...
  ]),
  {
    ...
    lifecycle: PRESENT_AT,
    signature:
      "export const meta = { name: string, description: string, phases?: Array<{ title: string; tier: string; detail?: string; model?: string }>, model?: string }",
    optionShape: null,
    constraints: [
      "must be the first statement",
      "name and description must be nonblank strings",
      "every declared phase must name a tier that the operator's model-tiers.json defines; a missing or unknown tier is rejected at parse time, naming the phase and the valid tier names",
