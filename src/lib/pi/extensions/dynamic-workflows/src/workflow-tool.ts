  ...
  script: Type.Optional(
    Type.String({
      description: [
        ...
        "The optional `agentType` option selects a named user or project definition that can bind tools, a model, and role instructions; use it only when its name and purpose are provided in context. Its bound model does NOT beat the phase tier — the phase tier decides the model for every agent in the phase, and a displaced `model`/`tier` is logged.",
        "Use plain JavaScript only; imports, require(), filesystem modules, Date.now(), Math.random(), and new Date() are unavailable.",
        "Use phase('Name'), agent(prompt, opts), parallel(arrayOfFunctions), pipeline(items, ...stages), log(message), args, cwd, process.cwd(), and budget. The workflow must call agent() at least once, and every agent() call needs a short unique label in opts, e.g. { label: 'researcher' }. Cost comes from the phase: an agent inside a phase runs on that phase's declared tier whatever it asks for, and an agent that runs outside any phase must pass its own { tier } — there is no default.",
