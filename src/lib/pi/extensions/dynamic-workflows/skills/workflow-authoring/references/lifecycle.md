...
`AGENT_CONTEXT_EXHAUSTED` is nonrecoverable and does not retry: a subagent that ran out of context (its own compaction-and-retry already failed, or its final turn was truncated) will hit the same wall again on a retry. By default that condition throws. Pass `onContextExhausted: "partial"` on that one `agent()` call to get a resolved value back instead — `{ complete: false, reason: "context_exhausted", text }`. `complete` is required, not an optional flag, precisely so a script that reads the result without checking it cannot mistake a cut-off answer for a finished one; check `complete` before treating the value as the agent's real output, and decide from the caller's side (re-decompose the task, widen the timeout/budget, or accept the partial) rather than retrying the same call. `compaction: false` on `agent()` opts a single long, serial call out of pi's own auto-compaction (e.g. a build-fix loop where only the current error matters); leaving it unset keeps today's inherited default.
